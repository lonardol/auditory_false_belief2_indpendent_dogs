...
```{r}
set.seed(1)
# Multiples of 3 from 60 to 240 (because 3 conditions)
multiples <- seq(from=63, to=240, by=3)
# Select every 6th value (because we don't want to run all 1000 simulations for 60 but for 10 values), so increments of 6 in sample size
n.subject <- multiples[seq(1, length(multiples), by = 6)]
...
breed<-c("akita_inu", "andalusian_hound", "basenji","beagle","canarian_warrenhound","greyhound","icelandic_sheepdog","samoyed", "siberian_husky","silken_windsprite","whippet", "jack_russel_terrier","parson_russel_terrier","border_terrier",
  "west_highland_white_terrier","yorkshire_terrier","fox_terrier","scottish_terrier","skye_terrier")
...
coefs <- c(
  "(Intercept)" = lp_FB_ind,
  "conditiontrue_belief" = lp_TB_ind - lp_FB_ind,
  "conditionknowledge_sound_control" = lp_KN_ind - lp_FB_ind,
  "sexM" = 0,
  "baited.fstgrey" = 0,
  "z.age" = -0.49
)
...
for (j in seq_along(n.subject)) {
  ...
  m.mat <- model.matrix(~ condition + sex + baited.fst + z.age, data = start.data)
  LP_fixed <- as.numeric(m.mat[, names(coefs), drop = FALSE] %*% coefs)

  # Export objects to workers (only small/light objects)
  clusterExport(cl, varlist = c("start.data","LP_fixed","breed","r.effects","r.slope.condition","contr","get_p"),
                envir = environment())
...
dat$breed <- factor(dat$breed) 
    ...
    full <- keepWarnings(glmer(
      choice ~ condition + sex + baited.fst + z.age +
        (1 + condition.dummy1.c + condition.dummy2.c | breed),
      family = binomial, data = dat, control = contr
    ))
    
    # drop1: main effect of condition 
    d2 <- tryCatch(as.data.frame(drop1(full$value, test = "Chisq")), error = function(e) NULL)

    # Means per condition 
    means <- with(dat, tapply(choice, list(condition), mean))
    # Ensure correct extraction order
    mc_fb_ind  <- means["false_belief"]
    mc_tb_ind  <- means["true_belief"]
    mc_kn_ind  <- means["knowledge_sound_control"]
    ...
    out <- list(
      ...
      lrt.p.drop1.sex = get_p(d2, "sex"),
      lrt.p.drop1.baited_fst = get_p(d2, "baited.fst"),
      lrt.p.drop1.z_age = get_p(d2, "z.age"),
  ...
  num_cols <- c("icpt","conditiontrue_belief","conditionknowledge_sound_control",
                "sexM","baited.fstgrey","z.age",
                "warns.full","re.sd", "lrt.p.drop1.condition", "lrt.p.drop1.sex","lrt.p.drop1.baited_fst","lrt.p.drop1.z_age",
...
save.image(file = here("power_analysis", "power_sim_Auditory_FB2_independent_dogs_only_workspace_parallel.RData"))
```
...
```{r}
all.res2 <- all.res.all.samples %>% filter(warns.full == 0) 
...
lrt.data2
save.image("power_sim_Auditory_FB2_independent_dogs_eval.RData")
